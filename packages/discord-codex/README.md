...
It does not patch or modify ChatGPT application files. It talks to the app-owned Codex task-follower IPC socket because that is the route that keeps the visible task in sync.

The ChatGPT migration preserved the `codex://` URL scheme, `com.openai.codex` bundle identity, `${TMPDIR}/codex-ipc` socket family, and the private follower protocol used here. Run `wakefield codex runtime` after desktop updates to check socket discovery and `initialize`; that non-mutating check does not validate target follower methods.
...
- The connector routes through the ChatGPT-owned Codex task follower IPC socket so the visible task stays in sync. If the app socket is missing or reports `no-client-found`, `auto` routing opens `codex://threads/<threadId>` to launch/load the app-owned follower, then polls the same follower IPC path for up to 30 seconds. While ChatGPT is still starting, it re-opens the deep link periodically because a cold launch can accept app activation before it is ready to navigate to the target task. If the follower still does not register, routing fails loudly and the message must not be silently sent through app-server.
...
- `app-server` and `remote-control` modes are diagnostics only. A separately managed daemon socket is not a ChatGPT desktop health signal, and daemon-routed turns are not guaranteed to update the visible task.
- If ChatGPT still does not register the task follower after the one-shot deep-link wake, the connector fails loudly instead of writing transcript files through a detached app-server process.
